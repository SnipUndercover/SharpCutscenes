...
/// </summary>
public sealed class ChapterSideStatistics
{
    #region Implementation Details
    // TODO: maybe some info about its parent?
    internal readonly AreaModeStats ChapterSideStats;

    internal ChapterSideStatistics(AreaModeStats chapterSideStats)
    {
    ...
    {
        private readonly ChapterStatistics Parent;
        private readonly Dictionary<ChapterSide, ChapterSideStatistics> Cache = [];

        internal StatisticList(ChapterStatistics parent)
        {
            Parent = parent;
        }
        ...
        /// </exception>
        public ChapterSideStatistics this[ChapterSide side]
        {
            get
            {
                if (Cache.TryGetValue(side, out ChapterSideStatistics? stats))
                    return stats!;

                if (side < ChapterSide.ASide || side > ChapterSide.CSide)
