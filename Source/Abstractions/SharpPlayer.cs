...
{
    // TODO: holdable helpers probably, like Pickup/Throw
    ...
    /// 
    /// <remarks>
    ///   This is done by checking whether her state is <see cref="PlayerState.StNormal"/>.<br/>
    ///   Allowing/disallowing movement sets Madeline's state to <see cref="PlayerState.StNormal"/> and <see cref="PlayerState.StDummy"/> respectively.<br/>
    ///   If you need more fine-grained control over states, check out <see cref="State"/>.
    /// </remarks>
    public bool CanMove
    {
        get => State == PlayerState.StNormal;
        set => State = value
    ...
    /// 
    /// <remarks>
    ///   Madeline can be in one state at any given time, and all vanilla states can be seen in <see cref="PlayerState"/>.
    /// </remarks>
    public PlayerState State
    {
        get => (PlayerState)ActualPlayer.StateMachine.State;
    ...
    {
        Audio.Play(SFX.game_gen_cassette_bubblereturn, Parent.Level.ActualLevel.Camera.Position + new Vector2(160f, 90f));
    ...
    // TODO: make separate intro type enum and test all Player.IntroTypes?
    ...
    ///   (default <see cref="Player.IntroTypes.Respawn"/>)
    /// </param>
    ...
    public void ChangeRoom(
        Vector2 position,
        Player.IntroTypes introType = Player.IntroTypes.Respawn,
        Vector2? spawnNear = null)
    {
        ...
        actualLevel.OnEndOfFrame += () => {
            actualLevel.TeleportTo(this, roomData.Name, introType, spawnNear ?? position);
        ...
        Vector2? spawnNear = null)
    {
        ...
        // add an event on frame end and change rooms there
        actualLevel.OnEndOfFrame += () => {
            actualLevel.TeleportTo(this, roomData.Name, introType, spawnNear);
        };
    }
    ...
    ///   Remember to put <see langword="yield"/> <see langword="return"/> before the call, else it won't run.
    /// </returns>
    public IEnumerator WaitUntilState(PlayerState targetState)
    {
